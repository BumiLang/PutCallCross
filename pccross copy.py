...
def get_opt_atm(yrmo, d_dir, eqvl_srch_rng = 5):
    ...
    opt['OPT_WIN'] = opt.groupby(['DATE', 'STRK'])['OPT_WIN'].ffill();
    opt['OPT_WIN_PRV'] = opt.groupby(['DATE', 'STRK'])['OPT_WIN'].shift(1);
...
def perf_pccrss(dt_strtg_begin_buf, dt_strg_end_buf, dt_react_obs, dt_react_hold, dp_lc, dp_pt, dp_pt_buf, unit, opt_atm):
    ...
    for idx, atm_i in strtgy_opt.iterrows():
        ...
        if (opt_win_prv_i != opt_win_i) \
            or (pos != 0 and ((pos_type != 'S' and opt_win_i == 'P') or (pos_type != 'L' and opt_win_i == 'C'))) :
